% max(+A, +B, +C)
% returns in C the biggest number between A and B
max(A,B,C) :-
  max_member(C,[A,B]).
...
% validate_input(+Min,+Max,+Value)
% checks if the input is within certain bounds
validate_input(Min,Max,Value) :-
  between(Min,Max,Value).
...
% calculates the number of pieces of a player given the board size
